...
class TektronixMSOModeError(Exception):
    """
...
ModeError = TektronixMSOModeError
"""
Alias for backwards compatibility
"""


class TektronixMDO30xx(VisaInstrument):
    """
    Driver for the MDO4000B, MDO4000, MSO4000B, DPO4000B and
    MDO3000 series Oscilloscopes.
    """
    ...
    def __init__(
        ...
        self.add_submodule("data", TektronixMSOData(self, "data"))

        self.add_submodule("waveform", TektronixMSOWaveformFormat(self, "waveform"))
        ...
        for channel_number in range(1, number_of_channels + 1):
            ...
            channel_module = TektronixMSOChannel(
                self,
                channel_name,
                channel_number,
            )
        ...
        self.add_submodule("trigger", TekronixMSOTrigger(self, "trigger"))

        self.add_submodule(
            "delayed_trigger",
            TekronixMSOTrigger(self, "delayed_trigger", delayed_trigger=True),
        )
...
class TektronixMSOData(InstrumentChannel):
    """
    ...
    def __init__(self, parent: Union[Instrument, InstrumentChannel], name: str) -> None:
        ...
        self.add_parameter(
            ...
            get_cmd="DATa:SOU?",
            set_cmd="DATa:SOU {}",
            vals=Enum(*TekronixMSOWaveform.valid_identifiers),
        )
...
class TekronixMSOWaveform(InstrumentChannel):
    """
...
class TektronixMSOWaveformFormat(InstrumentChannel):
    """
    ...
    submodule 'TekronixMSOWaveform' apply to
...
class TektronixMSOChannel(InstrumentChannel):
    """
...
class TektronixMSOHorizontal(InstrumentChannel):
    """
    ...
    def __init__(self, parent: Union[Instrument, InstrumentChannel], name: str) -> None:
        ...
        self.add_parameter(
            "record_length",
            get_cmd="HORizontal:RECOrdlength?",
        ...
        self.add_parameter(
            "sample_rate",
            get_cmd="HORizontal:SAMPLERate?",
            set_cmd="HORizontal:SAMPLERate {}",
            get_parser=float,
    ...
    def _set_scale(self, value: float) -> None:
        if self.mode() == "manual":
            raise TektronixMSOModeError("The scale cannot be changed in manual mode")

        self.write(f"HORizontal:MODE:SCAle {value}")


class TekronixMSOTrigger(InstrumentChannel):
    """
    ...
    https://download.tek.com/manual/MSO70000C-DX-MSO70000C-DX-MSO-MSO7000C-MSO-MSO5000B-Oscilloscope-Quick-Start-User-Manual-071298006.pdf
    """

    def __init__(self, parent: Instrument, name: str, delayed_trigger: bool = False):
        super().__init__(parent, name)
        ...
        self.add_parameter(
            "level",
            get_cmd=f"TRIGger:{self._identifier}:LEVel:{self._trigger_channel()}?",
            set_cmd=f"TRIGger:{self._identifier}:LEVel:{self._trigger_channel()} {{}}",
            get_parser=float,
        )
    ...
    def _trigger_channel(self) -> str:
        return self.ask(f"TRIGger:{self._identifier}:EDGE:SOUrce?")


class TektronixMSOMeasurementParameter(Parameter):
    """
    ...
    'TektronixMSOMeasurementStatistics'. Here we also find the method 'reset'
    to reset the values over which the statistics are gathered.
    """

    # pylint: disable=method-hidden
    def _get(self, metric: str) -> float:
        measurement_channel = cast(TektronixMSOMeasurement, self.instrument)
...
class TektronixMSOMeasurement(InstrumentChannel):
    """
    ...
    def __init__(self, parent: Instrument, name: str, measurement_number: int) -> None:
        super().__init__(parent, name)
        ...
        for measurement, unit in self.measurements:
            self.add_parameter(
                name=measurement,
                unit=unit,
                parameter_class=TektronixMSOMeasurementParameter,
            )
...
class TektronixMSOMeasurementStatistics(InstrumentChannel):
